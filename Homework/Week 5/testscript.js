...
d3.json("KNMI.json", function(error, json) {
    ...
    // initiate tooltip; x = date, y = max_temp
    focus.append("text")
        .attr("id", "x")
        .attr("class", "tooltip")
        .attr("dx", 8)
    ...
    svg.append("g").append("rect")
        ...
        .on("mousemove", function () {
            ...
            focus.select("#focusCircleY")
                .attr("transform",
                    "translate(" + x(d.date) + "," + y(d.max_temp) + ")");
            ...
            focus.select("#focusLineY")
                .attr("x1", 0).attr("y1", y(d.max_temp))
                .attr("x2", width).attr("y2", y(d.max_temp));
            ...
            focus.select("#y")
                .attr("transform",
                    "translate(" + x(d.date) + "," + y(d.max_temp) + ")")
                .text(d.max_temp + " degrees Celsius");
            focus.select("#y2")
                .attr("transform",
                    "translate(" + x(d.date) + "," + y(d.max_temp) + ")")
                .text(d.max_temp + " degrees Celsius");
