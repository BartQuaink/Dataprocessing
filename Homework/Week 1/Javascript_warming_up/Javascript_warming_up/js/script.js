...
// You will probably find "methods" like getElementById() very useful.
...
var nums = document.getElementById("ingredient-list").getElementsByTagName("li");
console.log(nums);

for (var i = 0; i < nums.length; i++)	{
	ApplePie.ingredients.push(nums[i].innerHTML);
}
...
// Therefore you can call it for any axis that you create and you neither have to make calculations by hand nor hardcode any values.
...
function createTransform(domain, range){
	...
	var base = domain[1] - domain[0];
	var beta = range[0];
	var alpha = (range[1] - range[0]) / (domain[1] - domain[0]) ;

	return function(x){
		return alpha * (x - base) + beta;
	};
}

// to use this for instance:
var transform = createTransform([15, 30], [15, 20]);
console.log(transform(30)); //should return 15!!
